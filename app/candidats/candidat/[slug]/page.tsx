...
async function getCandidat(slug: string) {
    try {
      ...
      const candidat = await sanityClient.fetch(query, {}, { next: { revalidate: 0 } });
      return candidat;
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="text-center py-4">
          <h1>
            PROFIL DU <span className="text-primary italic">CANDIDAT</span>
          </h1>
        </div>
        <Card className="w-full bg-background mx-auto mb-8 py-8">
          <CardContent className="flex flex-col items-start space-y-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="grid text-sm gap-1.5">
                ...
                <div className="flex items-center space-x-2 text-xs">
                  <p>{candidat.parti} &nbsp;{' | '} &nbsp; {candidat.age} ans &nbsp;{' | '} &nbsp; {candidat.lieu_de_naissance}</p>
                </div>
              </div>
            </div>
            ...
            <Button size="lg">
              <a href={candidat.programUrl} target="_blank">Télécharger le Programme</a>
            </Button>
          </CardContent>
        </Card>
      </main>
...
const serializers = {
  types: {
    block: (props: { node: { style: any, listItem: any }; children: React.ReactNode }) => {
